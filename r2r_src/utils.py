...
def read_img_features(feature_store, test_only=False):
    ...
    if not test_only:
        features = {}
        with open(feature_store, "r", encoding='unicode_escape') as tsv_in_file:     # Open the tsv file.
...
def add_exploration(paths):
    explore = json.load(open("data_v2/exploration.json", 'r'))
...
def ndtw_initialize():
    ndtw_criterion = {}
    scan_gts_dir = './data_v2/id_paths.json'
